...
function get_index()
{
  ...
  $file_template = "../{$dir_templates}/news_reader_index.html";
  ...
  foreach ($config_files_array as &$config_file)
  {
    ...
    {
      $page_content .= "<p class='clMediaName'><a href='./news_reader.php?varFuncId=headlines&varConfId={$conf_id}'>{$media_name}</a></p>\n";
    }
  }
...
function get_headlines()
{
  ...
  $file_template = "../{$dir_templates}/news_reader_headlines.html";
...
#exit;
  ...
  foreach ($articles_array as $article_block)
  {
    ...
    $page_content .= "<a href='./news_reader.php?varFuncId=article&varConfId={$conf_id}&varUrl={$headline_url}'>{$headline_name}</a>";
    $page_content .= "</p>\n";
  }
...
function get_article()
{
  ...
  $debug_file     = "/home/" . get_current_user() . "/mnt/data/Temp/debug_get_article.txt";
  $file_template = "../{$dir_templates}/news_reader_article.html";
...
#  }
  ...
  $article_body_content = preg_replace("/src='http:(.*?)'/s", "src='./image_server.php?varImageUrl=http:$1'", $article_body_content);
