...
else if ( $intResult == 2 )
{
  ...
  $strQuery = "update aa_client set price = 0.00, package = '00', payment_method = '00', payment_confirmed = 'N', date_requested = '0000-00-00', date_paid = '0000-00-00', date_completed = '0000-00-00', date_consulted = '0000-00-00' where idkey = '$strClientIdKey';";
  mysqli_query( $objConnection, $strQuery );
  mysqli_query( $objConnection, 'commit;' );
  mysqli_close( $objConnection );

  $strMailTo    = $strClientEmail;
  $strMailSubject  = "=?UTF-8?B?" . base64_encode( "Доступ к изменению метода оплаты за консультацию открыт" ) . "?=";

  $strMailData = fncGetTemplate( $strDirMailData . '/admin-client-paymenent-change.html' );
  ...
  $strContent = fncGetTemplate( $strDirResponses . '/admin-client-paymenent-change-passed.html' );
}
